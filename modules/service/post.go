...
func PostsCreate(db *sql.DB, params operations.PostsCreateParams) middleware.Responder {
	var exists bool
	var threadId int32
	var forumSlug string
	if exists, threadId, forumSlug = threadIsInDB(db, params.SlugOrID); !exists {
		return operations.NewPostsCreateNotFound().WithPayload(&models.Error{Message: "slug or id not found"})
	}
	...
	for _, post := range params.Posts {
		if inThread := postIsInThread(db, post.Parent, int64(threadId)); post.Parent != 0 && !inThread {
			return operations.NewPostsCreateConflict().WithPayload(&models.Error{Message: "parent is in another thread"})
		}

		if exists := userIsInDB(db, post.Author); !exists {
			return operations.NewPostsCreateNotFound().WithPayload(&models.Error{Message: "author not found"})
		}
...
func PostGetOne(db *sql.DB, params operations.PostGetOneParams) middleware.Responder {
	post := &models.Post{}
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return operations.NewPostGetOneNotFound().WithPayload(&models.Error{Message: "post not found"})
		}
		panic(err)
	}
...
func PostUpdate(db *sql.DB, params operations.PostUpdateParams) middleware.Responder {
	post := &models.Post{}
	err := db.QueryRow(queryGetPostById, &params.ID).Scan(&post.Author, &post.Created,
														  &post.Forum, &post.ID, &post.Message,
														  &post.Thread, &post.IsEdited)
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return operations.NewPostUpdateNotFound().WithPayload(&models.Error{Message: "post not found"})
		}
		panic(err)
	}
